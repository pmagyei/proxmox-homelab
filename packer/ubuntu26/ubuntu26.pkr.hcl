...
source "proxmox-iso" "ubuntu26" {
  ...
  task_timeout             = "10m"
  boot_wait                = "3s"
  vm_name                  = "template-ubuntu26-cloudinit"
  template_description     = "Ubuntu 26.04 Template - Built by Packer"
  os                       = "l26" # (Linux 2.6+)
  cores                    = 2
  memory                   = 4096
  cpu_type                 = "host"
  ...
  boot_command = [
    ...
    "c<wait>",
    "linux /casper/vmlinuz autoinstall ip=dhcp ds=nocloud-net\\;s=https://raw.githubusercontent.com/pmagyei/proxmox-homelab/refs/heads/master/packer/ubuntu26/http/ ---<enter><wait>",
